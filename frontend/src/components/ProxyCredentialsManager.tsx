...
import { Plus, Pencil, Trash2, Shield, Activity, Zap } from "lucide-react";
...
export function ProxyCredentialsManager({ open, onClose }: Props) {
  ...
  return (
    <Modal
      ...
      headerExtra={credentials.length > 0 ? (
        <button
          onClick={handleTestAll}
          disabled={testingAll}
          className="flex items-center gap-1 text-xs px-2 py-1 rounded bg-surface-3 text-gray-300 hover:text-gray-100 disabled:opacity-50"
            ...
            >
              ...
              <div className="flex items-center gap-1 ml-2">
                <button
                  ...
                  className="icon-btn text-gray-500 hover:text-accent"
                ...
                <button onClick={() => handleEdit(cred)} className="icon-btn text-gray-500 hover:text-gray-300" aria-label={`Edit ${cred.name}`} title="Edit">
                  <Pencil className="h-3.5 w-3.5" />
                </button>
                <button onClick={() => handleDelete(cred.id)} className="icon-btn text-gray-500 hover:text-red-400" aria-label={`Delete ${cred.name}`} title="Delete">
